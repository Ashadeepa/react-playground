...
function reducer(state: typeof initialState, action: ActionType) {
    switch (action.type) {
        ...
        case "sortBySymbol":
            return {
                ...initialState,
                coins: sortStrings([...state.coins], "symbol", !state.sortBySymbolDescending),
                sortBySymbolDescending: !state.sortBySymbolDescending,
                activeSort: "symbol"
            };
        case "sortByCurrentPrice":
            return {
                ...initialState,
                coins: sortNumbers([...state.coins], "currentPrice", !state.sortByCurrentPriceDescending),
                sortByCurrentPriceDescending: !state.sortByCurrentPriceDescending,
                activeSort: "currentPrice"
            };
        case "sortByOpeningPrice":
            return {
                ...initialState,
                coins: sortNumbers([...state.coins], "openingPrice", !state.sortByOpeningPriceDescending),
                sortByOpeningPriceDescending: !state.sortByOpeningPriceDescending,
                activeSort: "openingPrice"
            };
        case "sortByPriceInc":
            return {
                ...initialState,
                coins: sortNumbers([...state.coins], "priceInc", !state.sortByPriceIncDescending),
                sortByPriceIncDescending: !state.sortByPriceIncDescending,
                activeSort: "priceInc"
            };
        default:
            throw new Error();
        ...
        <table className="coin-table">
            <thead>
                <tr>
                    <TableHeaderSortable
                        caption="Coin Name"
                        isDescending={sortBySymbolDescending}
                        isActive={activeSort === "symbol"}
                        onClick={() => dispatch({ type: "sortBySymbol" })}
                    />
                    <TableHeaderSortable
                        caption={`Current Price (${currency})`}
                        isDescending={sortByCurrentPriceDescending}
                        isActive={activeSort === "currentPrice"}
                        onClick={() => dispatch({ type: "sortByCurrentPrice" })}
                    />
                    <TableHeaderSortable
                        caption={`Opening Price (${currency})`}
                        isDescending={sortByOpeningPriceDescending}
                        isActive={activeSort === "openingPrice"}
                        onClick={() => dispatch({ type: "sortByOpeningPrice" })}
                    />
                    <TableHeaderSortable
                        caption="Price Increase"
                        isDescending={sortByPriceIncDescending}
                        isActive={activeSort === "priceInc"}
                        onClick={() => dispatch({ type: "sortByPriceInc" })}
                    />
                </tr>
            </thead>
            <tbody>
